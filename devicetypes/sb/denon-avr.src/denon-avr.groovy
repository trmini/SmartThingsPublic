 ...
 *    Based on Denon/Marantz receiver by Kristopher Kubicki and Bobby Dobrescu
 *    SmartThings driver to connect your Denon/Marantz Network Receiver to SmartThings
...
metadata {
    definition(name: "Denon Marantz AVR", namespace: "SB", author: "Trang Luu") {
        capability "Actuator"
        capability "Switch"
        capability "Polling"
    ...
    preferences {
        ...
        input(title: "Marantz AVR version: ${getVersionTxt()}" ,description: null, type : "paragraph")
    }
    ...
    tiles(scale: 2) {
        multiAttributeTile(name:"multiAVR", type: "mediaPlayer", width: 6, height: 4) {
           tileAttribute("device.status", key: "PRIMARY_CONTROL") {
                attributeState ("paused", label: 'Off', backgroundColor: "#53a7c0", defaultState: true)
        ...
        standardTile("input5", "device.mp", width: 2, height: 2, decoration: "flat") {
            state "OFF", label: 'MPlayer', action: "mp", icon:"st.Electronics.electronics9", backgroundColor: "#FFFFFF",nextState:"ON"
            state "ON", label: 'MPlayer', action: "mp", icon:"st.Electronics.electronics9", backgroundColor: "#53a7c0", nextState:"OFF"
        }
...
def sPure() {
    setSoundMode("PURE DIRECT")
}

def setSoundMode(cmd) {
    //log.debug "Setting sound mode to '${cmd}'"
...
def update(controlName, controlState) {
    sendEvent(name: controlName, value: controlState)
}
